\section{案例分析} \label{Sec.cases}

本节介绍一些常用的程序设计结构在抽象机器 \amlh 上的实现。

%一个优秀的理论，定义下少数几个简单的公理与规则，各种丰富的重要而富有
%意义的定理引理可以由此推出。本节介绍的内容不属于 \noesishol 与
%\amlh 理论的定义，而是对 \noesishol 与 \amlh 的应用。
%这些应用体现了 \noesishol 与 \amlh 的灵活与魅力。

\subsection{列表的理解}

本节给出一个虚构理解与虚构基元的具体案例，列表的理解。
...
\[ \mathbf{V}_i \Rightarrow \mathbf{INJ}\ \mathbf{Li}_i\ \mathbf{Se}_i\ \mathbf{Sp}_i \]
\end{lemma}
\begin{proof} 
...
即{\phew}集同构于自然数集。先如下递归定义集合$S$上的列表$\mathbf{List}_S$集
...
下面是基元虚构律引入各种需要的功能，记号 $l_k$ 表示列表 $l$ 中的第$k$个元素，
...
表示该元素不需要使用任何实质的{\phew}而用零{\phew}$\mathrm{p}_0$即可，
不占用任何二进制位，在最后编译出的
目标程序中不需要也不会被表达，例如 C语言的void。
%却是一种合法的理解，可以参与 整个 $\amlh$ 的方方面面。
...
编译结果中实际不会出现参数$e$，在 AddN 内部参数$e$被作为常量优化。
\end{example}
...
{\phew}记录编号而其本质即是序列中编号上的元素。

至今尚未明确地具体定义{\phew}，而是留作为可定制的框架，令 $\rho(i)$ 表示用以表示数字i
的{\phew}，此外有简写$\rho_i = \rho(i)$
...
许多经典编程语言的设计都有函数参数的概念，它们或被叫做 lambda 函数或叫函数指针。
接下来的两节重点讨论函数参数在 $\amlh$ 上的实现。

函数参数分为两种，静态参数与动态参数。静态参数指在编译时可以确定函数参数所具体指代的过程，
更类似于模板的概念可以在编译时进行函数参数的带入；动态参数则相反，需要在运行时动态
调用函数指针。不同语言对此有不同的名称，另一种叫法是静态调用与动态调用。
%本节先论述静态参数，而下一节论述动态参数。

静态参数的理解，是 $\mathbf{I1}$，
一元过程的是 $\mathbf{I1}^{\mathrm{phenomenon} \rightarrow \mathrm{phenomenon}}$
二元过程是 \[\mathbf{I1}^{\mathrm{phenomenon} \rightarrow \mathrm{phenomenon} \rightarrow \mathrm{phenomenon}}\]
更高元以此类推。

$\mathbf{I1}^{\mathrm{phenomenon} \rightarrow \cdots \rightarrow \mathrm{phenomenon}}$
仅将{\phew} Noesis 对应到{\phew}过程，抽象语义只是{\phew}过程。
可以在同构条件中加入更多的约束，以进一步抽象。
%而很多时候期望对函数参数的抽象语义约束更多，
%诸如这种{\phew}过程具有在某些特定理解下 Noesis 对应到一个本质上的函数。
...
其中$f_p^0$ 表示$f_p$的伪{\phew}，占用0个二进制位而不存在于实际的编译结果中。
由 Noesis 对应同构的充分性定理 \ref{T.ptr.enough}
...
同理，可以分别虚构基元引入多元版本的 $\mathbf{Call}_2\ \cdots\ \mathbf{Call}_n$。
\end{defin}
...
将$f \proctr{\mathcal{N}|\mathcal{N}}{\K \T} f_e$ 作为过程条件的一部分。

\subsection{动态函数参数（虚函数表）}

$\amlh$ 上的动态函数只能在被确定的限定范围内动态。与许多传统编程语言类似，技术上 $\amlh$
不允许动态编译新的函数，因此动态函数的动态范围必须是确定且固定的。
%于是所谓的动态，其实是确定的虚函数表上的动态，是指动态地选择表中的不同函数。

类似于静态函数参数有单元素理解实现，动态函数参数由有限序列理解实现，一元的理解是
...
更高元类推。
而$\mathbf{SI}$的{\phew}所表达的序列中的序号实质是虚函数表的索引，在编译实现中将根据此索引查
虚函数表得到函数指针而动态地调用。

%关键难点与静态函数参数时一样，
%也是在于$\mathbf{SI}$的本质是{\phew}过程而需要有对此{\phew}过程的超越对应。
%而解决方法也是与静态函数类似的，通过在条件中加入对{\phew}过程的超越对应的约束。
%
%同样先给出一个样例以利于读者理解。
...
证明是与定义 \ref{Def.Call} 中几乎一样的，同样由Noesis 对应同构
的充分性定理 \ref{T.ptr.enough}
...
于是(1)被证明，于是可以虚构定义 $\mathbf{VCall}_1$。

同理虚构多元版本的 $\mathbf{VCall}_2\ \cdots\ \mathbf{VCall}_n$。
\end{defin}
...
%$\amlh$ 提供两种编写递归函数的方法，第一种严谨而难以使用，困难在于递归良好性的证明（wellfound），需要有经验的数学家巧妙地技巧；
...
现在回到$\amlh$上递归函数的构建，讨论本质对应具有如下形式的{\phew}过程的递归构建。
\[ \begin{split}
...
在假设中引入递归函数参数$g$，逐个对应本质的递归式得到递归{\phew}过程$F\ g$的超越对应
\[ \begin{split}
...
\ref{fibg3} 也成立，同样令 $g = \K \mathbf{0}$；
\ref{fibg4} 中，令
...
HOL 逻辑中已经定义了循环函数 $\Whilex : (\alpha \rightarrow \bool)
\rightarrow (\alpha \rightarrow \alpha) \rightarrow 
\alpha \rightarrow \alpha$ 
\[ \Whilex \coloneqq \forall P\ g\ x.\ \Whilex\ P\ g\ x = \xif P\ x 
\xthen \Whilex\ P\ g\ (g\ x) \xelse x \]
基础的循环构造作为一个基元指令可以被定义为
\[ \begin{split}
  \forall i\ c_p\ c_e.\ (\forall a_p\ a_e.\ &a_p \widesim{i} a_e 
\Rightarrow c_p\ a_p \widesim{\BoolI} c_e\ a_e) \Rightarrow \\
  \forall b_p\ b_e.\ &(\forall a_p\ a_e.\ a_p \widesim{i} a_e \Rightarrow
b_p\ a_p \widesim{i} b_e\ a_e) \Rightarrow \forall s_p\ s_e.\ s_p \widesim{i} s_e
  \Rightarrow \\&
  \Whileb\ c_p\ b_p\ s_p \widesim{i} \Whilex\ c_e\ b_e\ s_e 
\end{split} \]
即 $\Whileb$ 广义 Noesis 同构于 $\Whilex$，尽管这一广义的同构无法被
简单地用 Noesis 同构记号表示。

直接使用 $\Whileb$ 可能是困难的，可以由 $\Whileb$ 构建诸如列表的
map，reduce 等高级操作以简化。
