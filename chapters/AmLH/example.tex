...
这一节介绍一些常用的程序设计结构可以如何在抽象机器 \amlh 上实现。
...
这一节给出一个虚构理解与虚构基元的具体案例，列表的理解，具有实际意义。
...
于是表示这一个元素不需要使用任何实质的{\phew}除了独特的零{\phew}$\mathrm{p}_0$，
不占用任何二进制位，而在最后编译出的
...
诸如这种{\phew}过程具在某些特定理解下 Noesis 对应到一个本质上的函数。
...
假设中引入递归函数参数$g$，逐个对应本质的递归式得到递归{\phew}过程$F\ g$的超越对应
\[ \begin{split}
