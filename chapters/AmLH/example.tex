...
本节介绍一些常用的程序设计结构可以如何在抽象机器 \amlh 上实现。
...
本节给出一个虚构理解与虚构基元的具体案例，列表的理解，具有实际意义。
...
更类似于模板的概念可以在编译时进行函数参数的带入；而动态参数则相反，需要在运行时动态
...
问题在于$\mathbf{I1}^{\mathrm{phenomenon} \rightarrow \cdots \rightarrow \mathrm{phenomenon}}$
仅仅将{\phew} Noesis 对应到{\phew}过程，抽象语义只是{\phew}过程，而很多时候期望对函数参数的抽象语义约束更多，
...
其中$f_p^0$ 表示$f_p$的伪{\phew}，实际占用0个二进制位而实际不存在于编译结果中。
...
$\amlh$ 上的动态函数必须限定在确定的范围内的动态。与诸多其他编程语言类似，技术上 $\amlh$
不允许动态编译新的函数，于是动态函数可选的范围一定是有限而固定的。
