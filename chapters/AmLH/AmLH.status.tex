...
结构将状态数值化而纳入现象中，即令现象包含状态，那么现象上的函数或者说
...
每一个节点都是一个计算，
$\amlh$不显示地区分线程，而是基于依赖关系，依赖关系的存否
...
非常有趣的一点是，不同于普通的状态机理论，程序只能在变化的却始终唯一的
...
是有限映射（finite map）类型。有关有限映射的论述参见定义 \ref{Def.FM}。
\begin{align*} 
...
最后历史流是可以自然地纳入现象的，一个例子是

\begin{example}[历史流纳入现象]
    令类型 phenoval 表示原本的现象类型 phenomenon，而定义新的现象
    phenomenon 如下
    \[ \mathrm{phenomenon} \Coloneqq \mathbf{Phenomenon}\ 
    \mathrm{phenoval}\ \mathrm{history} \]
\end{example}

历史流融入现象的方式并非必须按这个例子来，但一定拥有函数
$\mathbf{P_H}$ 以得到现象中的历史流，
$\mathbf{P_V}$ 得到现象中的数值部分。
\begin{align*}
...
而历史是现象的一部分，历史融入后历史就被改变进而现象也被改变，
于是对状态的修改就会影响之后的现象，之后的现象也就依赖于这次状态的修改，
这样这状态修改就被依赖地存在而不能被优化。
亦即，在历史流的情形下，状态被纳入现象于是状态依赖就会产生计算依赖。
