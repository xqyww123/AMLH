\section{到智能合约的编译} \label{Sec.compile}

抽象机器 \amlh 的编译分为两步，首先程序被其定义展开成基元函数的组合，
成为一种中间表达（IR）。HOL 以文本格式输出 IR 给外部程序，外部程序
根据基元函数到目标平台指令集的映射，将 IR 翻译至目标平台。
用户程序到 IR 的编译是 HOL 上保障的等价变换，IR 到目标平台的编译
在 HOL 之外进行，是可能有错误的。
...
\amlhS 中每一个理解都具有确定的字节数，理解的值以此字节数排布在内存中
。计算时，所有小于等于4字节的{\phew}以 WASM 的 \Wi{32} 表示，大于4字节但
...
WASM 上的指令为各种类型设计了不同版本。例如整数加法有 \texttt{i32.add}
，\texttt{i64.add} 两种版本。\amlhS 中的基元运算也根据字节长度有不同
版本，例如 $\mbar{IAdd}\ n$ 表示 $n$ 个字节的整数加法基元函数，
具体参见表 \ref{tab.IC.primop2}。$n$ 个字节的基元函数对应 \Wtyp{n} 
...
WASM 采用一种树形结构表达， \amlhS 的 IR 的也同样以树形结构表达，与 WASM
的结构类似。

\amlh 中的 Let 结构表示本地变量，
\[ \mathrm{Let}\ p\ (\lambda x.\ p') \]
将编译为，赋值 $p$ 给本地变量 $x$，并在 $p'$ 的编译中，所有对
$x$ 的引用都编译为读取本地变量 $x$。
...
整个 $branch_A$ 与 $branch_B$ 都在分支块中，只有那些重复的计算被
置于之前的临时变量赋值中以重用。最后循环结构因 \amlhS 目前尚不支持。
表 \ref{tab.primop.WASM} 列出了基元函数到 WASM 指令的映射。
...
这一编译非常简洁，外部编译器所肩负的职责仅仅是对 \amlh 的 IR 进行
几乎指令对指令的翻译，不需复杂的编排或逻辑。外部编译程序可以
非常简短，以有效降低缺陷的可能性，并使对其的形式化验证更加容易。

编译好的用户程序需要进行最后的包装，在函数执行前插入处理来自智能合约
平台的输入的代码，并在函数之后增加对链数据写入指令的执行。
这两部分都是固定的，由算法 \ref{alg:package} 描述。
最后包装好的函数就可以被智能合约执行平台调用并能
