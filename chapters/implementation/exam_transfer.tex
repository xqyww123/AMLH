...
\end{align*}
帮助缩短公式长度。此程序会用到如下基元函数、预定义函数、常量
\begin{flalign}
...
输出结果占用的字节数为 $1 + \mathrm{PointerSize}$。当输入与输出的大小被确定，这一函数的调用接口即被确定。
...
下面将证明转账函数 $\mathbf{Transfer}_\mathrm{p}$ 对调用条件 $P$ 的不变性，即
...
此证明再次体现了本文的思想，不是直接证明程序的性质，而是证明程序
