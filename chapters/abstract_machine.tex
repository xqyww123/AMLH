...
首先是基于$\lamst$的且无状态纯粹数值计算的抽象机$\amlN$，
然后加入状态的考虑发展到$\aml$，也是$\amlh$不考虑实现的纯抽象模型。
而后论述将$\aml$实现在HOL定理证明器的HOL逻辑上，
...
可以将状态抽象为各个分立无关的系统资源，那么状态就是各资源到其当前状态的映射。

\begin{equation} \label{Def.Stat}
\begin{split} 
& \textnormal{SysStatus} = \textnormal{ResKey} \mapsto \textnormal{ResStatus} 
 \\
& \textnormal{ResKey} : \textnormal{string} \quad\quad \textnormal{ResStatus} : \textnormal{number} 
\end{split}
\end{equation}

其中 $\textnormal{ResKey}$ 表示一切的系统资源，用字符串表示，；$\textnormal{ResStatus}$ 
表示资源的状态，用自然数表示。一切资源的状态都是可以用一个自然数表示的，
尽管这个自然数可能非常大。事实上整个计算机的内存状态都可以用一个巨大的自然数表示。
这自然数有多大并不重要，因为形式逻辑地分析实际不需要将其具体数值计算出来。

有两种方法增强 $\amlN$ 以拥有系统状态的功能，一种是将计算与状态分离，将在接下来详细讲述；
另一种是利用历史流，将在\ref{Sec.hisFlow}节详细论述。

状态与计算分离，是使用请求-响应模型（Request-Response），
将程序分割成下层平台与上层计算过程两个部分，令生存期管理（Life time manage）、
数据持久化（Data persistence）由下层的平台管理，
而计算过程只负责计算响应与对状态的修改指令，而后由下层平台执行状态修改指令再发生实际的
状态修改。
...
\begin{defin}[状态转移指令]
    \begin{equation}
        \begin{split}
    \mathrm{transition\_op} & \Coloneqq \mathbf{TransitionOp}\ \mathrm{string}\ \natn \\
    \mathrm{transition\_ops} & \Coloneqq \mathrm{transition\_op}\ \mathrm{list}
        \end{split}
    \end{equation}
\end{defin}


\subsection{单源单汇有向无环网络流的数值化系统状态模型} \label{Sec.hisFlow}
