...
\begin{defin}[到$\alpha$的理解] 类型 $\itp{\alpha}$ 是现象到
...
理解及其机制可以认为是 $\amlh$ 上的类型系统。之所以说“可以”，是因为理解及其机制实际
实现了类型系统的功能，但并不严格地符合类型系统的定义；而$\amlh$上类型系统的功能
被理解及其机制实现，所以实际上并不需要类型系统。严格地来说理解及其机制是
类型系统的替代。
...
\begin{defin}[超越对应关系] 定义三元关系 $ p \widesim{i} e$ 表示现象与本质在某种理解
    下的的对应关系。
\begin{equation}
     p \widesim{i} e \coloneqq p \in \mathbf{Sp}_i \land (\mathbf{Tr}_i\ e = p)
\end{equation}
\end{defin}
...
\begin{prooftree}
    \AxiomC{$\cdots$} 
    \RightLabel{对于某个成立的 $p \widesim{i} e$\quad\quad(类型证明率)} \doubleLine
...
\begin{defin}[理解的合并] 一个到 $\alpha$ 的理解$i_a$与到 $\beta$ 的理解$i_b$可以合并成
...
这意味着一个计算客体可以同时具有多种理解，而现象上的一次计算可以同时带有多个含义。
...
\begin{proof}
\begin{align*}
    \mathbf{V}_{a\cdot b} =\ & (\forall e_a\ e_b.\ e_a \in \mathbf{Se}_a \quad \land \quad
    e_b \in \mathbf{Se}_b \quad \land \quad 
    (\mathbf{Li}_a\ e_a = \mathbf{Li}_b\ e_b) \quad \Rightarrow \\
    & \quad\quad\quad \mathbf{Li}_a\ e_a \in \mathbf{Sp}_a \cap \mathbf{Sp}_b \quad \land
    \quad (\mathbf{Tr}_a(\mathbf{Li}_a\ e_a) = e_a) \quad \land \quad
    (\mathbf{Tr}_b(\mathbf{Li}_b\ e_b) = e_b)) \\
    \mathbf{V}_a =\ & (\forall e_a.\ e_a \in \mathbf{Se}_a \Rightarrow
      \mathbf{Li}_a\ e_a \in \mathbf{Sp}_a \quad \land \quad 
      (\mathbf{Tr}_a(\mathbf{Li}_a\ e_a) = e_a)) \\
    \mathbf{V}_b =\ & (\forall e_b.\ e_b \in \mathbf{Se}_b \Rightarrow
      \mathbf{Li}_b\ e_b \in \mathbf{Sp}_b \quad \land \quad 
      (\mathbf{Tr}_b(\mathbf{Li}_b\ e_b) = e_b)) \\
    & \mathbf{V}_a;\ \mathbf{V}_b;\ e_a \in \mathbf{Se}_a;\ e_b \in \mathbf{Se}_b;\ 
    \mathbf{Li}_a\ e_a = \mathbf{Li}_b\ e_b \quad \vdash \quad
...
\begin{prooftree}
\AxiomC{$\Gamma \vdash p \widesim{i_1} e_1$}
\AxiomC{$\Gamma \vdash p \widesim{i_2} e_2$}
    \RightLabel{(合并率)}
\BinaryInfC{$\Gamma \vdash p \widesim{i_1 \cdot i_2} (e_1,\ e_2)$}
\end{prooftree}
...
类型证明率，合并率，合并分解率构成了值的类型演算。

\begin{defin}[过程的超越对应关系] 定义4元符号 $f_p \prec i | j \succ f_e$ 表示1个参数
的过程的超越对应， $n+2$ 元符号 $f_p \prec i_1 | \cdots | i_n \succ f_e$ 表示 $n$ 个参数的过程的
超越对应。
\begin{equation} \begin{split}
    (f_p:\mathrm{phenomenon}\rightarrow\mathrm{phenomenon})\ & \prec i | j \succ
    (f_e:\alpha \rightarrow \beta) \coloneqq \\
&(\forall a_p\ a_e.\quad a_p \widesim{i} a_e \Rightarrow f_p\ a_p \widesim{j} f_e\ a_e) \\
    (f_p:\mathrm{phenomenon}\rightarrow \cdots \rightarrow \mathrm{phenomenon})\ & 
    \prec i | \cdots | l \succ
    (f_e:\alpha \rightarrow \cdots \rightarrow \delta \rightarrow \epsilon) \coloneqq \\
    (\forall a_p\ a_e \cdots d_p\ d_e.\quad a_p \widesim{i} a_e \Rightarrow &
    \ b_p \widesim{j} b_e \Rightarrow \cdots \Rightarrow f_p\ a_p\ \cdots\ d_p
    \widesim{l} f_e\ a_e\ \cdots\ d_e)
\end{split} \end{equation}

    特别的 $p \proctr{i} e \coloneqq p \widesim{i} e$
