...
\subsection{基元指令（Primitive Operations）}
...
事实上可以证明，对于任意的超越对应关系，一定存在一个现象函数满足此对应关系。
...
\[ \begin{split} \forall f_e\ i\ \cdots\ k\ l\ cond.\ 
 \mathbf{V}_i\ \land\ &(\forall a_e\ \cdots\ c_e.\ cond\ a_e\ \cdots\ c_e \Rightarrow
   \mathbf{Se}_l\ (f\ a_e\ \cdots\ c_e) ) \Rightarrow \\
...
\begin{proof} \begin{gather*}
  p \widesim{i} e = p \in \mathbf{Sp}_i \land (\mathbf{Tr}_i\ e = p) \\
\mathbf{V}_i = (\forall e.\ e \in \mathbf{Se}_i \Rightarrow \mathbf{Li}_i\ e \in
    \mathbf{Sp}_i \land (\mathbf{Tr}_i(\mathbf{Li}_i\ e) = e)) \\
\mathbf{V}_i = (\forall e.\ e \in \mathbf{Se}_i \Rightarrow \mathbf{Li}_i\ e \widesim{i} e)
\end{gather*}

即，目标命题得证，存在 $p = \mathbf{Li}_i\ e$
...
且 $\Gamma_\sim = \{a_p \widesim{i} a_e,\ \cdots\ ,\ c_p \widesim{k} c_e,\ 
cond\ x_e,\ (\forall a_e\ \cdots\ c_e.\ cond\ x_e \Rightarrow x_e \in \mathbf{Se}_l),\ 
\mathbf{V}_l\}$

\begin{prooftree}
\AxiomC{$\Gamma_\sim \vdash x_e \in \mathbf{Se}_l \land \mathbf{V}_l$}
\RightLabel{引理 \ref{lem.1}}
...
\UnaryInfC{$(\forall a_e\ \cdots\ c_e.\ cond\ x_e \Rightarrow x_e \in \mathbf{Se}_l),\ 
\mathbf{V}_l \vdash \exists f_p.\ f_p \proctr{i|\cdots|k|l}{cond} f_e$}
\UnaryInfC{$\forall f_e\ i\ \cdots\ k\ l\ cond.\ 
 \mathbf{V}_i\ \land\ (\forall a_e\ \cdots\ c_e.\ cond\ a_e\ \cdots\ c_e \Rightarrow
   \mathbf{Se}_l\ (f\ a_e\ \cdots\ c_e) ) \Rightarrow$} \noLine
\UnaryInfC{$\exists f_p.\ f_p \proctr{i|\cdots|k|l}{cond} f_e$}
\end{prooftree}
\end{proof}
