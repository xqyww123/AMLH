...
记号 $f_p \proctr{a_e \sim i|j}{cond} f_e$ 表示1个参数的且以
$cond$为条件的过程的 Noesis 同构，记号
$f_p \proctr{e_1 \sim i_1 | \cdots | e_n \sim i_n|j}{cond} f_e$ 表示 $n$ 个参数的过程的且条件为
$cond$的 Noesis 同构。

\begin{equation} \begin{split}
    p \proctr{i}{cond} e \coloneqq &\ (cond \Rightarrow p \widesim{i} e) \\
    (f_p:\mathrm{phenomenon}\rightarrow\mathrm{phenomenon})\ & 
    \proctr{a_e \sim i|j}{cond}
    ...
    \proctr{a_e \sim i | \cdots | c_e \sim k | l}{cond}
...
$\proctr{a \sim i | \cdots | c \sim k | l}{cond}$ 中的理解 $i \cdots k$
与本体 $a \cdots c$ 可以相互依赖。
首先本体可以依赖于本体，这是显然的。
理解可以依赖于理解，可以由一个理解计算得出。序列的增加是一个例子
\[ \mathbf{Append} \proctr{x \sim i | l \sim \ListI\ i | 
\ListI i}{\lambda x\ l.\ \T} (::) \]
将 Noesis 同构记号展开后
\[ \forall x_p\ x\ i\ l_p\ l.\ x_p \widesim{i} x \Rightarrow
  l_p \widesim[2]{\ListI\ i} l \Rightarrow \mathbf{Append}\ x_p\ l_p
  \widesim[2]{\ListI\ i} x::l \]
是合理合法的 HOL 逻辑表达式。
理解可以依赖于本体，若 $\mathcal{N}^{\leq}_n$ 表示所有不超过 $n$ 的自然
...
记号展开后是
\[ \forall a_p\ a\ n\ b_p\ b.\ a_p \widesim[2]{\mathcal{N}^\leq_n} a
\Rightarrow b_p \widesim[2]{\mathcal{N}^\leq_a} b \Rightarrow
\mathbf{NSub}\ a_p\ b_p \widesim[2]{\mathcal{N}^\leq_a} a - b \]
也是合法的。本体可以依赖于理解，
\[ f_p \proctr{a\sim \mathcal{N}^\leq_n|\mathcal{N}^\leq_n}
{\K \T} (\lambda a.\ n - a) f_e \]
展开后是
\[ \forall a_p\ a\ n.\ a_p \widesim[2]{\mathcal{N}^\leq_n} a
...
本体互相依赖尽管这是可以做到的。于是一般在 Noesis 同构的记号中省略
...
展开后的表达是
\[ \forall n\ a_p\ a_e\ b_p\ b_e.\ a_p \widesim[2]{\mathcal{N}^\leq_n}
 a_e \Rightarrow b_p \widesim[2]{\mathcal{N}^\leq_n} b_e \Rightarrow
 a_e \geq b_e \Rightarrow
 \mathbf{NSub}\ a_p\ b_p \widesim[2]{\mathcal{N}^\leq_n} a_e - b_e\]
基本充分利用调用条件 $cond$ 就不怎么需要本体与理解互相依赖。

\begin{theo}[\noesishol 上的调用律、抽象律、一阶同构引入律、一阶同构削除律、
记号律、记号削除律]
在 Noesis 对应的定义 \ref{Def.TR} 与 Noesis 同构的定义 \ref{def.proctr} 
下，Noesis 系统的调用律、抽象律、一阶同构引入律、一阶同构削除律、
记号律、记号削除律，以 HOL 上的定理的形式出现。
...
\begin{minipage}{\linewidth}\begin{prooftree}
  ...
  \UnaryInfC{$\Gamma \vdash \mathrm{Let}\ p\ (\lambda x_\mathrm{p}.\ p')
    \widesim{i} \mathrm{Let}\ e\ (\lambda x_\mathrm{e}.\ e')$}
\end{prooftree}\end{minipage}
