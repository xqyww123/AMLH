...
\begin{defin}[类型集$\PiE$]
字符串集合 $\hat{U}$ 为用于表示类型的字母表
$\hat{U} \subseteq \String$，
函数$\xa : \hat{U} \mapsto \mathbb{N}$表示类型的元数（Arity），
$\hat{U}$ 中元数为0的子集$\hat{U}_\mathrm{v}$：
...
\begin{algorithmic}[1]
\Require 类型匹配系统 $(V,X) \in \EQs$，
实例化函数 $f : \Uv \rightarrow \PiE$ 表示变量到值的对应
\Ensure $\IM\ f\ (V,X) \in \EQs$
\State $\{\} \rightarrow X'$
\For {$(x,y) \in X$}
...
接下来证明 $\mathrm{SolveM}$ 的停机性。为此需要引入一些别的工具。

\begin{defin}[类型方程组的重量]
类型方程组的重量函数 $\EQS : (\PiE \times \PiE) \rightarrow
    \mathbb{N} \times \mathbb{N}$
    \[ \EQS X = (\abs{\UV X}, \sum_{(x,y) \in X} \TS(x) + \TS(y)) \]
$\abs{\UV X}$ 表示集合 $\UV X$ 的大小，故而 $X$ 必须是有限的。
\end{defin}
...
(\mathrm{SolveM_r}((V_x,X_x),(V_o,&X_o)) = ((V_x',X_x'),(V_o',X_o')))
    \vdash\\ (\EQS X_x' < \EQS\ &X_x)\ \lor\ ((V_x',X_x') = \MNS)
\end{split} \]
\begin{proof}
...
\noindent 在 $n = 0$ 时，由引理 \ref{T.cah} 性质1
不存在 $q_1,\ q_2$ 使得 $n = \max(\mathrm{h}\ q_1,\ \mathrm{h}\ q_2)$

\noindent 在 $n = 1$ 时，由引理 \ref{T.cah} 性质1
