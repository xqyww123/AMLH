...
Curry-Howard 同构下程序即是证明的编程语言与定理证明工具踩在了点上。特别是 Coq，
完美地实现了演绎定理以构建程序。

只是 Coq、Agda、Idris、F* 等等基于构造演算的系统还是将思维停留在了盒子中，
本文的贡献在于，在演绎定理以构建程序的思路下，{\bfseries 演绎以构建程序的定理，
根本不必须是 $x : \tau$ 的形式，不必须是二元的类型关系}。Curry-Howard 同构自然
只在类型系统中显现，但单纯为了构建程序而演绎的定理，不必须限制在类型关系，
只需要定理中包含程序就行了，{\bfseries 就可以是别的，呈现更多性质的扩展}，
而在此扩展下会有对应的 Curry-Howard 同构的扩展。
...
在本文，演绎以构建程序的定理，是程序到其抽象意义的对应关系，$x \widesim{i} \epsilon$，
其中 $\widesim{\cdot}$ 是一个三元关系，$x$ 表示程序部分，名叫 {\it 现象}
({\it phenomenon, φαινόμενον})，
$i$ 表示理解方式叫做 {\it 理解}({\it understanding, nóēsis, νόησῐς})，
$\epsilon$ 表示程序 $x$ 在理解 $i$ 下的抽象对应，表示抽象意义，
名叫 {\it 本体}({\it noumenon, νoούμενον})，
$\widesim{\cdot}$ 关系叫做{\it Noesis 对应}({\it Noesis correspondence})或者强行翻译成
...
抽象对应 $\epsilon$ 是定理证明器上任意的数学对象，是数学友好的并易于分析与证明的，
比如集合与数学映射。抽象意义的分析与证明是容易的，因为它们是数学友好的数学对象。
Noesis 定理保障了程序实现与其抽象意义的对应，
而后用户只需证明抽象意义的性质以证明程序的性质，比如证明作为一个值的抽象对应的
自然数 $7$ 是质数以证明这个值表达一个质数，或者关于参数 $n$ 的程序的抽象对应
$2^n + 1$ 不总是质数以证明此程序并不总是输出质数。
是去分析抽象的数学对象，而不是面对一段现实的复杂的程序。

若将程序的设计也由此定理证明器上的形式语言描述，描述为一种抽象的数学对象，
就与程序的抽象意义位于同一个形式语言的空间中，
那么证明程序的抽象意义与此设计的抽象表达的相等性，就能彻底证明程序实现对于此设计的正确性。
而这种证明是容易的，因为程序的抽象意义是数学友好的，易于分析的。

这也许有点像指称语义（Denotation Semantic），有类似的意味，也许可以参考以助于理解，
但理论内容是完全不同，不应被混淆。
...
可以用一些例子更细致地说明 Noesis 系统，及其是如何演绎定理以构建程序的。
...
即过程调用 $f(x)$ 的本体为 $g\ \epsilon$。此肯定前件律的应用是程序 $f(x)$ 的构建。用户的编程行为，既是利用现象操作作为本体的理念，也是在理念
的世界中变换抽象的本体以构建作为现象的程序。

一个本体可以表现不同现象，这是自然的。
而更有趣的，在本文的 Noesis 系统中，一个现象可以在不同的理解下下对应
...
单独的指称并没有任何意义，{\it 理解}可以肆意捏造以将任意的现象对应到
任何本体，例如一些诡异的理解可以将黑指代为白，而将鹿指代为马。
意义体现在同构(Homomorphism)，无论怎样肆意设定的{\it 理解} $i$，当其确定
，若能证明对
一段程序过程$p$输入理解$i$下指代到某个本体$\epsilon$的现象会产生
理解$j$下指代到本体$f(\epsilon)$的输出，此函数的抽象意义
就被理解 $i,\ j$ 确定，即本体 $f$。
例如某个定理表明名叫 Add 的程序输入理解 $\mathcal{N}$ 下
对应 $x$ 和 $y$ 的值，$x, y$ 是自然数，就能输出在解释 $\mathcal{N}$
...
上的程序的常量集与指令集。
一个形式系统的所有定理都来自于其公理按在其定义的演绎律的演绎。
...
可以实现在此目标执行环境上，于是程序的基元指令与常量表达也可以实现在那目标执行环境上，
...
而本文的情况，先是在某种解释或某种解释的同构下证明程序$x$对应于
...
本文的新思路应与 Curry-Howard 同构有什么联系。Coq 证明工具
...
而本体是定理。既然 Noesis 对应是类型关系的扩广，那似乎暗示着 
Curry-Howard 同构也应存在着某种扩广，也许是沟通起程序与建构主义式的
抽象构造。似乎显然的一点是，一切被建构的构造，都有一个程序对应，数学是，证明是，
一块布丁也是。

现在放下理念的美好回到现实，讨论如何演绎 Noesis 理论以在实际的工业生产中
构建程序，图 \ref{fig:total-frame} 为整体实施框架。
...
本工作使用 HOL 交互式定理证明工具作为定理推理引擎，对其详细介绍在 \ref{Sec.ITP} 节中进行。

首先在 HOL 定理证明工具上构造一系列定义、谓词、定理以及证明策略，将
Noesis 系统实现在 HOL 交互式定理证明器的证明系统中，进而可以使用 HOL 逻辑
表述现象、理解、理念。其中现象与理解将被特别地定义，而
本体 noumenon 就是 HOL 逻辑中任意的数学对象(mathematical object)，
例如数学映射、集合、有限域。于是可以有效且自然地证明本体的任意性质，
因为这本体是就是 HOL 逻辑上自然存在的数理对象。
...
但如前所述，\amlh 的基元指令集与常量被设定的如此接近目标执行环境以至于编译后端的工作
...
\draw [align=center] (A) ([turn]90:2) node[right]{等价变换\\展开成基元指令\\与常量的表达};
    \draw [align=center] (2.7,0) node[right] {中间表达};
...
的介绍。
\item[第 \ref{Ch.AmLH} 章] 细致地论述 Noesis 系统的定义，其在 HOL 上的实现，以及相关的各种细节。
\item[附录 \ref{Ch.ES}] 论述编辑壳层，因内容过多且与 Noesis 系统相对无关故置于附录。
  仅给感兴趣的读者，可以选择性阅读或跳过。
...
\item[第 \ref{Ch.conclusion} 章] 总结本文工作，与其他现有方案比较，特别是与依赖类型系统
  比较。
\item[附录 \ref{Ch.lambda}] 包括了 λ 演算粗浅的介绍与定义，可供数学背景较弱的读者参考。
\item[附录 \ref{Ch.example.transfer}] 是第 \ref{Ch.Implementation} 章的扩充，
  给出了一段相对完整的代币合约转账部分在 \amlhS 上具体的构建过程与编译。
  因内容过于细致放于附录。
\end{itemize}
