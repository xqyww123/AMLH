...
根本不需要是 $x : \tau$ 的形式，不需要是二元的类型关系}。Curry-Howard 同构自然
只在类型系统中显现，但单纯为了构建程序而演绎的定理，不需要限制在类型关系，
只需要定理中包含程序就行了，{\bfseries 就可以是别的，呈现更多性质的扩展}，
而在此扩展下会有相应的 Curry-Howard 同构的扩展。
...
一个本体可以表现不同现象，这是自然的。
而更有趣的，在本文的 Noesis 系统中，一个现象可以在不同的理解下对应
...
，若能证明程序过程$p$ 当输入理解$i$下对应到本体$\epsilon$的现象时，
会输出在理解$j$下对应到本体$f(\epsilon)$的现象，则此函数的抽象意义
就被理解 $i,\ j$ 确定，即本体 $f$。
...
现在放下理念的美好回到现实，讨论如何在实际的工业生产中演绎 Noesis 理论以
