...
\end{array}\\
\begin{array}{lcl}
\Has\ (\TC c\ i\ j)\ x\ k &\coloneqq& (c,\ \Li_i k) \in \Dom x\\
...
有一些显然的定理
\begin{gather*}
    \Read\ t\ (\phi \fupdate \Write\ t\ k\ v)\ k' =
    (\xif k = k' \xthen v \xelse \Read\ t\ \phi\ k') \\
t_1 \neq t_2 \Rightarrow (\Read\ t_1\ (\phi \fupdate \Write\ 
t_2\ k\ v)\ k' = \Read\ t_1\ \phi\ k') \\
    \ReadZ\ t\ (\phi \fupdate \Write\ t\ k\ v)\ k' =
    (\xif k = k' \xthen v \xelse \ReadZ\ t\ \phi\ k') \\
t_1 \neq t_2 \Rightarrow (\ReadZ\ t_1\ (\phi \fupdate \Write\ 
t_2\ k\ v)\ k' = \ReadZ\ t_1\ \phi\ k') \\
  \ReadZ\ (\TC\ c\ i\ j)\ \emptyset\ k = \Tr_j\ (\PhV\ 0\ \Si_j)
\end{gather*}
\end{defin}
...
$\mathbf{Read0}\ t$ & $\mathbf{Read0}\ t \proctr{\OneI\ x|i|j}
  {\K (\K \T)} \ReadZ\ t$ &
对给定的链数据 $x$，表标识 $c$，若链数据$x$的表$c$中存在$k$键则读取，
  否则返回 $j$ 理解下的0值\\ \hline
$\mathbf{Write}\ t$ & $\mathbf{Write}\ t \proctr{i|j|\CWI}
  {\lambda k\ v.\ \T} \Write\ t$ &
写入链数据的表$c$的键$k$为$v$的操作，产生写入命令 \\ \hline
$\mathbf{Cart}$ & $\mathbf{Cart} \proctr{i|j|i\times j}{\lambda
  x\ y.\ \T} (\lambda x\ y.\ (x,\ y))$ & 合并两个值而构造元组 \\ \hline
$\mathbf{Cart}$ & $\mathbf{Cart} \proctr{\ListI\ \CWI|i|\RsI\ i}
