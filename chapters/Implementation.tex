\chapter{基于 Noesis 逻辑的智能合约生成工具 \Eamlh }\label{Ch.Implementation}

工具 \Eamlh 是第 \ref{Ch.AmLH} 章的程序开发技术在智能合约场景的实现，能生产
较高执行性能的易于被形式化分析与验证的智能合约。且 \Eamlh 使用 HOL 定理证明器
作为证明引擎，允许用户使用 HOL 定理证明器完成对抽象语义的证明，进而完成最终
对智能合约的证明。

工具 \Eamlh 的架构已在图 \ref{fig:total-frame} 中指出，其中 \Eamlh 使用的用于智能合约场景
的 Noesis 理论版本在 \ref{Sec.amlhS} 节论述；\Eamlh 的编译后端在 \ref{Sec.compile} 节
论述；编辑壳层因为内容过多且与 Noesis 理论联系较弱放在附录 \ref{Ch.ES}；最后的
编辑前端目前的实现非常简陋，仅支持命令行交互，且代码量很少约400行本文不详细论述。

\section{抽象机 \amlh 的智能合约版本 \amlhS } \label{Sec.amlhS}
...
\begin{defin}[{\phew}的定义] 类型 phenomenon 表示{\phew}
\[ \begin{split}
...
在内存上的直接拼接构成的{\phew}。$\PhP p$ 表示对{\phew} $p$ 的指针。
\[ \begin{array}{lcllcl}
...
\end{array} \]
$\PX\ (\PhP p)$ 刻意留作未定义。PointerSize 为常量，固定为智能合约执行环境
的指针位宽，一般为32。
{\phew}的分割工具，函数 $\PL,\ \PU$
...
\end{gather*}
{\phew}集的 0 元素是 $\mathbf{PhV}\ 0\ 0$，
占用0个比特位的值，且其是合法的。
\end{defin}

\begin{defin}[\amlhS 上的理解]
\amlhS 上理解的定义基于定义 \ref{Def.itp} ，额外固定了理解的{\phew}集
的位数目。
    \[ \begin{split}
...
$\mathbf{Si}_i$ 表示理解 $i$ 的{\phew}集的比特数。
\end{defin}
...
链数据是表标识与键的元组到值的有限映射，其中表标识以 string 类型表示，键和值均
以 phenomenon 类型表示。 
...
\textbf{理解} & \textbf{抽象语义集} & \textbf{{\phew}集} & 
...
\item[注1] 理解 $i \times j$ 的{\phew}集是 $\{\PhS p_1\ p_2
...
\newline \newline
\begin{threeparttable}
\centering \caption{ \amlhS 中实现的理解（续）} \label{tab.IC.noesis2}
...
%值得一提的是
分支 $\mathbf{If}$ 的定义在表 \ref{tab.IC.primop2} 中列出
，不具有简单形式的 Noesis 同构形式，而是
\[ \begin{split}
...
\end{split} \]
  ...
  再由理解 $\BoolI$ 前提 $p_c \widesim {\BoolI} P$ 下 $P = \T$ 进而
    有 $p_a \widesim{i} \epsilon_a$ 于是命题得证。
若 $\PX p_c \neq 1$ 则由 $\mathbf{If}$ 定义，有
   $ \mathbf{If}\ p_c\ p_a\ p_b = p_b $，
  再由理解 $\BoolI$ 前提 $p_c \widesim {\BoolI} P$ 下 $P = \F$ 进而
...
所有 \amlhS 的智能合约所暴露的可供外部调用的接口应是一种{\phew}函数，
...
诸如$\mathbf{IAdd}\ n$ 操作要求给定运算的位宽。
\amlhS 提供基于这些基元函数构建的高层函数库。
...
\mathbf{AddN_S}\ 42\ m\ x \proctr{\NatSegI\ m|\NatSegI\ \max(42,m)}
  ...
  \end{prooftree}
\end{center}
编译时，$\mathbf{AddN_S}\ 42\ 666\ x\ y$ 根据定义展开
\[ \begin{split}
