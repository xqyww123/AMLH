...
工具 \Eamlh 是第 \ref{Ch.AmLH} 章的程序开发技术在智能合约场景的实现，能够生产
...
编辑前端目前的实现非常简陋，仅支持命令行交互，且代码量很少约400行不去详细论述。
...
\amlhS 上理解的定义基本基于定义 \ref{Def.itp} ，只是固定了理解的{\phew}集
的位数目。
    \[ \begin{split}
...
链数据是作为字符串的表标识与作为{\phew}的键的元组到作为{\phew}的值的有限映射。
...
每次都显式地给出位宽。\amlhS 提供基于这些基元函数构建的高层函数库。
