...
的所有键与值，并类似地有 $\Dom_{(\mathbf{TC}\ c\ i\ j)}\ \phi$，$
  \Ima_{(\mathbf{TC}\ c\ i\ j)}\ \phi$
  \[ \begin{array}{lcl}
    \Dom_c\ \phi &=& \{x\mbar (c,x) \in \Dom \phi\} \\
    \Ima_c\ \phi &=& \{\phi\ (c,x) \mbar (c,x) \in \Dom \phi \} \\
    \Dom_{(\mathbf{TC}\ c\ i\ j)}\ \phi &=& \{\epsilon\mbar (c,\Li_i 
    \epsilon) \in \Dom \phi\} \\
    \Ima_{(\mathbf{TC}\ c\ i\ j)}\ \phi &=& \{\Tr_j(\phi\ (c,\Li_i 
...
i}{\lambda l\ x.\ \T} (::)$ \\ \hline
  $\mathbf{Write}\ t$ & 虚构定义自同构 $\mathbf{Write}\ (\mathbf{TC}\ 
  c\ i\ j)\proctr{i|j|\CWI}{\lambda k\ v.\ \T}
(\lambda k\ v.\ ((c,\ \mathbf{Li}_i\ k),\ \mathbf{Li}_j\ v))$ \\ \hline
  $\mathbf{Read}\ t$ & 虚构定义自同构 $\mathbf{Read}\ (\mathbf{TC}\ 
  c\ i\ j) \proctr{\OneI\ x|i|j}
  {\lambda x\ k.\ (c,\ \Li_i k) \in \Dom x}
  (\lambda x\ k.\ \Tr_j(x (c,\ \Li_i k)))$ \\ \hline
  $\mathbf{Has}\ t$ & 虚构定义自同构 $\mathbf{Has}\ (\mathbf{TC}\ c\ i 
  \ j) \proctr{\OneI\ x|i|\BoolI}{\K (\K \T)}
  (\lambda x\ k.\ (c,\ \Li_i k) \in \Dom x)$ \\\hline
...
是 HOL 系统库中列表的增加函数，$1::[2]$ 表示 $[1,2]$ \\ \hline
  $\mathbf{Write}\ t$ & $\mathbf{Write}\ (\mathbf{TC}\ c\ i\ j)
\proctr{i|j|\CWI}{\lambda k\ v.\ \T}$\newline$
(\lambda k\ v.\ ((c,\ \mathbf{Li}_i\ k),\ \mathbf{Li}_j\ v))$ &
写入链数据的表$c$的键$k$为$v$的操作，产生写入命令 \\ \hline
  $\mathbf{Read}\ t$ & $\mathbf{Read}\ (\mathbf{TC}\ c\ i\ j)
\proctr{\OneI\ x|i|j}{\lambda x\ k.\ (c,\ \Li_i k) \in \Dom x}$
  \newline$(\lambda k.\ \Tr_j(x\ (c,\ \Li_i k)))$ &
对给定的链数据 $x$，表标识 $c$，读取链数据$x$的表$c$的$k$键 \\ \hline
$\mathbf{Has}\ t$ & $\mathbf{Has}\ (\mathbf{TC}\ c\ i\ j)
