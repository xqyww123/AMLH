...
\begin{prooftree}
\AxiomC{$\Gamma, x : \sigma \vdash M : \tau$} \RightLabel{(抽象律)}
\UnaryInfC{$\Gamma \vdash \lambda x{:}\sigma. M : \sigma \leftarrow \tau$}
\end{prooftree}
\end{minipage}
...
\end{minipage}\begin{minipage}[b]{0.5\linewidth}
\begin{prooftree}
\AxiomC{$\Gamma \vdash (\lambda (v:\sigma)\ M)\ x : \tau$}
\RightLabel{($\beta$规约)}
\UnaryInfC{$\Gamma \vdash M[v/x] : \tau$}
...
$\Lambda t_1\ \Lambda t_2\ \cdots\ \Lambda t_n\ b$，
\[ \Pi_* = \bnf{\Pi \mbar \forall U\ \Pi_*} \]
记号 $\forall \tau_1\ \tau_2\ \cdots\ \tau_n.\ b$ 表示
$\forall \tau_1\ \forall \tau_2\ \cdots\ \forall \tau_n\ b$
...
\begin{defin}[β规约] β规约$\breduce$是集合$L$即 λ 表达式上满足
    \[ (\lambda x\ b)\ a \breduce b[x/a] \]
且在下述规则下闭合的最小关系
\begin{align*}
    &P \breduce P'& &\Rightarrow& &\forall x \in V:\quad\lambda x.P 
    \breduce \lambda x. P'&\\
    &P \breduce P'& &\Rightarrow& &\forall Z \in L:\quad P\ Z
    \breduce P'\ Z&\\
    &P \breduce P'& &\Rightarrow& &\forall Z \in L:\quad Z\ P
    \breduce Z\ P'&
\end{align*}
\end{defin}
