...
基元指令有两种引入方法。第一种是基于现象的定义 \ref{Def.phenomenon}，
具体地给出现象上函数的定义，再由 Noesis 对应的定义 \ref{Def.TR}
与 Noesis 同构的定义 \ref{def.proctr} 证明此函数满足某个或某些
 Noesis 对应同构，一个确定的现象函数可以在不同的理解诶下
具有多个 Noesis 对应同构，于是由此可以定义具有多个本体或者说多个
意义的函数。例如在现象上完整定义的加法函数既可以 Noesis 同构到
自然区间的加法，或者有限域的加法。

第二种是根据预先期望的 Noesis 对应同构，不给出明确的定义，而是
证明对于给定的 Noesis 对应同构必定存在一个现象函数满足此同构，
进而由此引入。这一方式叫做 {\it 虚构引入}。
事实上可以证明，对于任意的 Noesis 对应同构，只要满足一些很容易的条件，
就一定存在一个现象函数满足此对应同构。
进而虚构基元指令实际根本不用具体地描述计算过程，可以根据其意义直接引入。
也根本不需要关注计算内容，只要得到了 Noesis 对应同构定理，
就可以直接由组合律去使用。
...
\begin{theo}[基元虚构定理]
给定所期望的本体$f_e$，参数的理解$i\ \cdots\ k$，返回值的理解$l$，过程执行的条件$cond$，
只要 $l$ 有效，且 $cond$ 足以限定返回值的本体 $f_e\ a_e\ \cdots\ c_e$到其理解$l$的本体集
$\mathbf{Se}_l$内，则必定存在一个现象上的过程 $f_p$，且满足期望的
Noesis 对应同构。
...
不可能直接证明某个现象在该理解下的超越对应，但依旧可以通过虚构基元指令
与虚构常量的方式引入关于此虚构理解的基元指令。

至此，抽象机 \amlh 就论述完成。这种机器基于 \noesishol 系统，主要加入了
基元指令与常量的引入方法。基元指令、常量、理解，均可以用两种方式引入。

第一种方法给出具体的包括完整的对现象的计算的定义，根据定义证明其具有
 Noesis 同构、Noesis 对应、或者是合法的理解。一个现象或现象过程可能
 在多种理解下有多种 Noesis 对应或同构，即有多个本体对应。

对于复杂的结构与过程这种方法可能是困难的，第二种方法使用虚构基元指令、
虚构常量、虚构理解，在给定的期望的 Noesis 同构、Noesis 对应、理解的
本体集下，证明及其简单与基本的几个条件，就可以虚构得到这些定理。
虚构得到的基元指令、常量、理解只能拥有其虚构中给出的确定的一种
Noesis 同构、Noesis 对应与本体集，虽然不能有多重对应，但非常简单易用。
