...
具体地给出现象上函数的定义，再由 Noesis 对应的定义 \ref{Def.TR} 证明此函数满足某个或某些
 Noesis 对应同构，一个确定的现象函数可以在不同的理解下
具有多个 Noesis 对应同构，于是由此可以定义具有多个本体或者说是多个
意义的函数。例如在现象上完整定义的加法函数，既可以 Noesis 同构到
自然数区间的加法，也可以是有限域的加法。

第二种是根据预先期望的 Noesis 对应同构，不给出明确的定义，而是
构造一个存在性命题，证明必定存在一个现象函数满足此同构，
进而由此引入。这一方式叫做 {\it 虚构引入}。
事实上可以证明，对于任意的 Noesis 对应同构，只要满足一些很容易的条件，
就一定存在一个现象函数满足此对应同构。
进而虚构的基元指令实际根本不用具体地描述计算过程，可以根据其意义直接引入。
...
不可能直接证明某个现象在该理解下的 Noesis 对应，但依旧可以通过虚构基元指令
