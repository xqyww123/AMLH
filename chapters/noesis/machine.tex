...
常量、基元指令、理解均有两种引入方法，第一种是普通地给出具体的计算表达时来定义。
...
此外另一种定义方法分别是，常量定理，基元指令虚构，理解虚构。
三者均不需要给出具体的表达，而只是由所期望的抽象语义，证明必定存在这样的常量、基元指令、
理解满足所期望的抽象语义，进而引入。
...
基元指令有两种引入方法。第一种是基于{\phew}的定义 \ref{Def.phenomenon}，
具体地给出{\phew}上函数的定义，再由 Noesis 对应的定义 \ref{Def.TR} 证明此函数满足某个或某些
 Noesis 对应同构，一个确定的{\phew}函数可以在不同的理解下
具有多个 Noesis 对应同构，于是由此可以定义具有多个抽象语义或者说是多个
意义的函数。例如在{\phew}上完整定义的加法函数，既可以 Noesis 同构到
自然数区间的加法，也可以是有限域的加法。
...
一般而言$\amlh$的实现中{\phew}集是同构于自然数集的一阶无穷，这就给$S_e$的选择带来很大的空间。
