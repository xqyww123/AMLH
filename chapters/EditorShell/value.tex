...
\begin{defin}[非量化值常量、非量化值变量、非量化值] \label{D.Value}
所有的非量化值常量构成集合 $L_c$ ，所有非量化值变量构成集合 $L_v$ 
$L_c^2$
\begin{align*} 
L_c &= \bnf{\hLc : \PiE}&L_v &= \bnf{\hLv : \PiE}&\end{align*}
所有的非量化值构成的集合 $\LE$ 是所有满足如下规则的最小集合
\[ \begin{array}{rcl}
...
\underline{\Lambda \tau\ x}\in \LE^2&
\end{align*}
递归定义非量化值的值重量（value only weight）
...
根据定义域区分。
递归定义非量化值的类型重量（type only weight）$\vwt : \LE \rightarrow \mathbb{N}$
\[ \begin{array}{lclcl}
...
\vwt \underline{(\ \Lambda\ v\ x\ )} & = & 1 + \vwt(x) & \when & 
    \underline{(\ \Lambda\ v\ x\ )} \in \LE\\
\end{array} \]
最后是非量化值的总重量 $\vW : \LE \rightarrow \mathbb{N}$
\[ \vW x = \vw x + \vwt x \]
...
\begin{align*} 
    ...
    \VW z &= \Vw z + \Vwt z
\end{align*}
同样可以通过定义域区分这些量化值的重量与值的重量，额外的，因为任何
非量化值也都是一种量化值 $\LE \subseteq \LE^2$，
任何非量化值被视作量化值时的各种重量跟作为非量化值时的各种重量是相同的，
故不会引起冲突。
\end{defin}

\begin{defin}[$\LE$ 与 $\LE^2$ 的类型] \label{D.type}
递归函数 $\type : \LE \rightarrow \PiE$ 计算非量化值的类型
\[ \begin{array}{lcl}
...
这一定义也给出了计算非量化值的类型的递归算法 \ref{alg:type} type，与
计算量化值了类型的算法 \ref{alg:type2} $\type^2$。

\begin{algorithm}
\caption{非量化值的类型 type} \label{alg:type} \algcase
\begin{multicols}{2}
\begin{algorithmic}[1]
\Require 非量化值 $x \in \LE$
\Ensure $\type x$
...
非量化值集 $\LE$ 上也具有 β 规约，与定义 \ref{D.breduce} 是一样的。
...
\begin{defin}[非量化值的调用与可调用性] \label{D.V.call}
若非量化值 $f$ 与非量化值 $a$ 满足条件 $f \vcallable a$则被叫做
    $f$ 可被 $a$ 调用。
...
进而有非量化值的调用函数 $(\vcall) : \LE \times \LE \rightarrow \LE$
\[ f \vcallable a \Rightarrow (f \vcall a = \B\underline{(\ f\ a\ )}) \]
同样此处的符号 $(\vcall)$ 不应被混淆因为定义域不同。
\end{defin}

\begin{algorithm}
\caption{非量化值的可调用测试 suit} \label{alg:suit}
\begin{algorithmic}[1]
\Require 非量化值 $f,\ x \in \LE$
\Ensure Bool 非量化值 $f \vcallable x$
    \State 输出 $(\type f\ \text{是函数类型}\quad\land\quad \Dom(\type f) =  \type x)$
    \TimeComplexity $O(1)$
\end{algorithmic}
\end{algorithm}
\begin{algorithm}
\caption{非量化值的调用 vCall} \label{alg:vCall}
\begin{algorithmic}[1]
\Require 非量化值 $f,\ x \in \LE\quad$满足 $f \vcallable x$
\Ensure $f \vcall x$
...
\caption{非量化值的特化 Inst} \label{alg:Inst}
\begin{multicols}{2}
\begin{algorithmic}[1]
\Require 特化范围 $V \in \powerset(\Uv)$，类型特化函数 $f : \Uv \rightarrow \PiE$，非量化值 $x \in \LE$
\Ensure $\Inst_V\ f\ x$
    \Case {$x$}
    \When {$\underline{(\ v:\tau\ )}$}
...
\end{algorithm}
\begin{algorithm}
\caption{非量化值量化 MakeQV} \label{alg:MakeQV}
\begin{algorithmic}[1]
...
至此定义完了非量化值的调用运算，可以由此执行大量的函数调用。接下来将定义
量化值的调用运算。首先从量化值的特化开始。

\begin{defin}[非量化值的特化] \label{D.Inst}
递归定义非量化值的特化函数 
$\Inst : (\Uv \rightarrow \PiE) \rightarrow \LE \rightarrow \LE$，
但其值域 $\LE$ 是需要证明，这将在引理 \ref{D.Inst.LE}
...
非量化值的特化的递归定义 \ref{D.Inst} 同时也给出了非量化值特化的算法，
形式地写作算法 \ref{alg:Inst} Inst。
...
\begin{defin}[非量化值的集的调用]
非量化值的集的调用是函数 $(\vcalls) : \powerset(\LE) \times 
    \powerset(\LE)\rightarrow \powerset(\LE)$
...
\begin{defin}[量化值的调用] \label{D.V.Call}
...
量化值对应的特化集合即是将$z_1,\ z_2$对应的特化集的所有非量化值
一一尝试调用得到的结果构成的集合。
