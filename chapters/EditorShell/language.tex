...
在 SML 语言中，全称量化的特化只发生在函数，参数是无法全称量化特化的，
且无法产生具有所谓任意类型 $\forall x.\ x$ 的值。\ES 中参数是可以
自由地量化特化的，具有任意类型 $\forall x.\ x$ 的值可以调用于任何函数。
另外 SML 的类型系统并非完全的 System F，而是 Hindley-Milner
算法下被限制的，因为完全的 System F 中类型无法被自动推导，
而 Hindley-Milner 算法给出了在近似 System F 类型系统下的
自动类型推导算法。XEE!!! 引用！
但在 \ES 的情况，类型是不需要被自动推导的，因为根本没有文本，没有
