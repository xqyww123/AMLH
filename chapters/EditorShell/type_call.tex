\subsection{类型的调用} \label{S.T.Call}
...
原本的 λ2 演算要求显式地实例化组合双方的类型，直到实例化成不具有全称量化的形式才可以进行
...
\Ensure $(q,f) \in \PiAE \times (\Uv \rightarrow \PiE)$ 满足 
$\QI q_1 \cdot \QI q_2 = \QI q\ \land\ \QB q = \inst f\ (
    \Dom^\rightarrow \QB q_1)$
\Statex 或者 $\QI q_1 \cdot \QI q_2 = 
\emptyset$ 时输出 {\it 无解} 。
...
\begin{defin}[全称量化类型的调用] \label{D.QCall}
    由定理 \ref{T.QCall} 可以定义全称量化类型的调用。先定义可调用性
    $\tcallable : \PiAE \times \PiAE \rightarrow \Bool$
    \[ q_1 \tcallable q_2 = (\QI q_1 \cdot \QI q_2 \neq \emptyset) \]
算法 \ref{alg:QCall} QCall 的有无解即是对 $\tcallable$ 的计算，
    $\mathrm{QCall}(q_1,\ q_2)$有解时 $q_1 \tcallable q_2 = \T$ 无解时
$q_1 \tcallable q_2 = \F$

    从 $\PiAE \times \PiAE$ 到 $\PiAE$ 的函数 $(\cdot)$ 表示可调用时的
    全称类型调用
    \[ \forall q_1,\ q_2 \in \PiAE.\ (q_1 \tcallable q_2) 
\Rightarrow \QI (q_1 \cdot q_2) = \QI q_1 \cdot \QI q_2\]
    算法 \ref{alg:QCall} QCall 即是计算 $(\cdot)$ 在可调用时的结果。
\end{defin}
